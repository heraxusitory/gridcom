...
namespace App\Http\Controllers\PriceNegotiations;
...
class PriceNegotiationController extends Controller
{
    ...
    {
        try {
            $price_negotiations = PriceNegotiation::query()->with(['positions'])->get();

            $price_negotiations->map(function ($negotiation) {
                $negotiation->order = $negotiation->order()->first();
                return $negotiation;
            });
    ...
    {
        try {
            $price_negotiation = PriceNegotiation::query()->with(['positions'])->findOrFail($price_negotiation_id);
