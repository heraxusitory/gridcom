...
class PriceNegotiationController extends Controller
{
    ...
    {
        try {
            return response()->json(['data' => (new CreatePriceNegotiationService($request))->run()]);
        } catch
    ...
    public function update(UpdatePriceNegotiationFormRequest $request, $price_negotiation_id)
    {
        try {
            ...
            return response()->json(['data' => (new UpdatePriceNegotiationService($request, $price_negotiation))->run()]);
        } catch
        (ModelNotFoundException $e) {
            return response()->json(['message' => $e->getMessage()], 404);
        } catch (\Exception $e) {
    ...
    public function downloadFile(Request $request, $price_negotiation_id)
    {
        ...
        try {
            ...
            if (Storage::exists($price_negotiation->file_url)) {
                return response()->download(Storage::path($price_negotiation->file_url), 'some-file-name');
            }
            return response('', 204);
        } catch
        (ModelNotFoundException $e) {
