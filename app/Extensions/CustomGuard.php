...
//            dd($this->decodedToken);
    ...
    {
//        dd($this->user, is_null($this->user));
//        if (array_key_exists('sub', $this->decodedToken))
...
//        dd($user);
    ...
    //custom
    public function getRoles($resource): \Illuminate\Support\Collection
    {
        $token_resource_access = (array)$this->decodedToken->resource_access;
        if (array_key_exists($resource, $token_resource_access)) {
            $token_resource_values = (array)$token_resource_access[$resource];
            ...
            if (array_key_exists('roles', $token_resource_values)) {
                return collect($token_resource_values['roles']);
            }
        }
        return collect([]);
    }
