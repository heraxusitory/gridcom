...
class OrderTransformer extends TransformerAbstract
{
    ...
    public function transform(Order $order)
    {
        ...
        $customer = [
            'organization' => ['name' => $order->customer?->organization?->name],
            'work_agreement' => ['number' => $order->customer?->contract?->number],
            'work_type' => optional($order->customer)->work_type,
            'object' => ['name' => optional($order->customer?->object)->name],
            'sub_object' => ['name' => optional($order->customer?->subObject)->name],
            'work_start_date' => optional($order->customer)->work_start_date,
            'work_end_date' => optional($order->customer)->work_end_date,
        ];

        $provider = [
            'contr_agent' => ['name' => optional($order->provider?->contr_agent)->name],
            'provider_contract' => ['number' => optional($order->provider?->contract)->number],
        ...
        $contractor = [
            'contr_agent' => ['name' => optional($order->contractor?->contr_agent)->name],
